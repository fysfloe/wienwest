...
jQuery(function($) {
  ...
  var $quill_text = $('#quill-text');

  if($quill_text.length > 0) {
    var quill = new Quill('#quill-text', {
      placeholder: 'Wos wüst?',
      theme: 'snow'
    });

    var $quill_hidden_field = $($quill_text.data('field'));

    $('.ql-editor').html($quill_hidden_field.val());

    quill.on('text-change', function() {
      console.log($('.ql-editor').html());
      $quill_hidden_field.val($('.ql-editor').html());
    });
  }
