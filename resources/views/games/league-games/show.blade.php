...
@section('content')
	<div class="league-games games">
		@if(Auth::user()->hasRole('admin'))
			@if ($errors->league_game_edit_result->get('home_team') || $errors->league_game_edit_result->get('away_team'))
				<div class="alert alert-danger">
					<strong>Herst!</strong> Fehler beim Ergebnis eintragen! Versuch's noch einmal.<br><br>
					<ul>
						@foreach ($errors->league_game_edit_result->all() as $error)
							<li>{{ $error }}</li>
						@endforeach
					</ul>
				</div>
		...
		<div class="lineup row">
			...
			@if(Auth::user()->hasRole('admin') && !isset($past_game))
				<a class="btn btn-default" href="{{ route('lineup', array('league_games', $game->id)) }}"><i class="fa fa-btn fa-users"></i> Aufstellung</a>
			@endif
		</div>
		@if(!isset($past_game))
