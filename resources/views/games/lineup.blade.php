...
@section('content')
    @if (count($errors->lineup) > 0)
        <div class="alert alert-danger">
            <strong>Herst!</strong> Mach' das gfälligst richtig!<br><br>
            <ul>
                @foreach ($errors->lineup->all() as $error)
                    <li>{{ $error }}</li>
                @endforeach
            </ul>
        </div>
    ...
    <div class="form-group">
        ...
        <div class="col-md-6">{!! Form::select('mode', array('4-2-3-1' => '4-2-3-1', '4-3-3' => '4-3-3', '4-4-2' => '4-4-2', '4-1-3-2' => '4-1-3-2', '3-4-1-2' => '3-4-1-2'), isset($lineup->mode) ? $lineup->mode : '4-2-3-1', array('class' => 'form-control')) !!}</div>
    </div>
    ...
    <div class="participants lineup" data-mode="@if(isset($lineup->mode)){{ $lineup->mode }}@else{{'4-2-3-1'}}@endif">
        <h3>Aufstellung</h3>
            ...
            @include('games.lineups.droppable.3-4-1-2', isset($lineup) ? array('lineup' => $lineup) : array())
